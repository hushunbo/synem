...
\begin{figure}[H]
\centering
    \subfloat[Estimated $\overline{Y}(x), 0\leq x \leq 255$, according to eq. (\ref{eq:E_Step_stats}). The height of each box is proportional to $\sigma_{Y}$]{\label{fig:T1T2}\includegraphics[width=0.75\linewidth]{./images/t1_t2_transfer_brainweb.png}}\\
    \subfloat[Estimated $\overline{Z}(x), 0\leq x \leq 255$, according to eq. (\ref{eq:E_Step_stats}). The height of each box is proportional to $\sigma_{Z}$]{\label{fig:T2T1}\includegraphics[width=0.75\linewidth]{./images/t2_t1_transfer_brainweb.png}}\\
    \caption{Estimated transfer functions, according to eq. (\ref{eq:E_Step_stats}) using the Brainweb template where the T1 image is $I$ and the T2 image is $J$
    (since the two images are perfectly aligned, the trasnformations $\phi_{I}, \phi_{J}$ are identities). }
\label{fig:transfers}
\end{figure}
