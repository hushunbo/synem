...
def _tile_plot(imgs, titles, share_ax_with, **kwargs):
    """
    Helper function
    """
    ...
    if share_ax_with is not None:
        ...
        ax = []
        for i in range(n):
            new_ax = fig.add_subplot(n, 1, 1+i, sharex=share_ax_with, sharey=share_ax_with)
            ax.append(new_ax)
    else:
        fig, ax = plt.subplots(len(imgs), 1, sharex=True, sharey=True)
